...
module NavigationHelper
  ...
  class Navigation	
	...
	#--Added on 07/08/08 by Matt Milosavljevic --
	#determines if the subtitles should replace the main link text
	def wants_replaced_text?
    ...
    private
      ...
      def construct(nav, section)
        ...
        if nav.wants_hover_text?
          link = link_to(text, send("#{section.to_s.downcase}_path"), :title => strip_tags(SUBTITLES[section]) )
		elsif nav.wants_replaced_text?
		  link = link_to(SUBTITLES[section], send("#{section.to_s.downcase}_path"), :title => strip_tags(SUBTITLES[section]) )
